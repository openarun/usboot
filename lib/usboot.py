...
class Usboot:
    ...
    def raw_format(self, usb_path, fstype, label):        
        ...
        def thread_run():                            
            ...
            if os.geteuid() > 0:
                launcher='pkexec'
                output = Popen([launcher,'/usr/bin/python', '/usr/lib/usboot/raw_format.py','-d',usb_path,'-f',fstype, '-l', label], shell=False, stdout=PIPE)   
            else:
                output = Popen(['/usr/bin/python', '/usr/lib/usboot/raw_format.py','-d',usb_path,'-f',fstype, '-l', label], shell=False, stdout=PIPE)                
    ...
    def raw_write(self, source, target):        
        ...
        def thread_run(): 
            try:          
                ...
                if os.geteuid() > 0:
                    launcher='pkexec'
                    output = Popen([launcher,'/usr/bin/python', '/usr/lib/usboot/raw_write.py','-s',source,'-t',target], shell=False, stdout=PIPE)
                else:
                    output = Popen(['/usr/bin/python', '/usr/lib/usboot/raw_write.py','-s',source,'-t',target], shell=False, stdout=PIPE)                    
                while flag == True:
                    try:                
...
if __name__ == "__main__":
    ...
    def usage():
        print "Usage: usboot [--debug] -m [format|iso]              : mode (format usb stick or burn iso image)"
        print "       usboot [--debug] -m iso [-i|--iso] iso_path"
        print "       usboot [--debug] -m format [-u|--usb] usb_device "
    ...
    Usboot(iso_path, usb_path, filesystem, mode, debug)
